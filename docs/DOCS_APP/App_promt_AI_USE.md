...
class IStockItemRepository(Protocol):
    ...
    async def get(self, product_id: str, warehouse_id: str, lot_id: str | None) -> StockItem | None: ...
...
class InventoryUseCases:
    ...
    async def post_movement(self, payload: dict, idem_key: str) -> StockMove:
        ...
        try:
            ...
            # Update stock item
            item = await self.item_repo.get(move.product_id, move.warehouse_id, move.lot_id)
            if not item:
                item = StockItem(
            ...
            # Read-back
            verified = await self.item_repo.get(move.product_id, move.warehouse_id, move.lot_id)
    ...
    async def get_stock_level(self, product_id: str, warehouse_id: str, lot_id: str | None = None) -> StockLevel:
        try:
...
class StockItemModel(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "product_id", "warehouse_id", "lot_id", name="uq_stock_item"),
