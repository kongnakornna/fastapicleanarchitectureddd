# 📦 สร้าง Module `tenant_context` แบบเต็ม (ห้ามไฟล์ว่าง)

ใช้วิธีเดิม (PowerShell here-string ผ่าน batch wrapper) เพราะปลอดภัยกว่า batch ล้วน

---

## 📄 ไฟล์: `create_tenant_context.bat`
...
## 📄 ไฟล์: `create_tenant_context.ps1`
...
### 1. สร้าง 2 ไฟล์
วาง **`create_tenant_context.bat`** และ **`create_tenant_context.ps1`** ไว้ในโฟลเดอร์เดียวกัน (root ของโปรเจกต์ ระดับเดียวกับ `app/`)

### 2. รัน
Double-click **`create_tenant_context.bat`**
...
**รวม 25 ไฟล์ — ไม่มีไฟล์ว่าง** ✅

---

## 🔑 จุดสำคัญของ Module นี้

| จุด | รายละเอียด |
|---|---|
| **`contextvars`** | propagate context แบบ async-safe ไม่ใช้ global state |
| **`schema_name`** | `f"tenant_{tenant_id}"` สำหรับ PostgreSQL schema-per-tenant |
| **`redis_namespace`** | `f"t:{tenant_id}:{key}"` สำหรับ Redis prefix |
| **`kafka_topic`** | `f"t.{tenant_id}.{topic}"` สำหรับ Kafka prefix |
| **3-branch** | Use cases: `StandardException` → `DomainError` → `Exception` |
| **2-branch** | Repositories: `StandardException` → `Exception` |
| **never-raise** | Caches: log warning แล้ว return `None` / `None` |
| **Re-export** | `domain/entities.py` re-export `RequestContext` เพื่อความสะดวก |
