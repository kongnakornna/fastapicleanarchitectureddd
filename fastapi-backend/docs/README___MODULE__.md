...
> Layer: 2 | Prefix: mon | Version: 1.0.0

## Purpose

TH: อธิบายวัตถุประสงค์ของ module money
EN: describe purpose of money module
...
a fresh stack migrates itself.
    ...
    make migration m="create_money_model"   # autogenerate
    make migrate                                 # apply

A new model must be imported in `migrations/env.py` **and** added to its
`_ = [...]` list — see `migrations/versions/db/REGISTER_money.md`.
...
|---|---|---|---|
| POST   | /api/v1/money/      | สร้างใหม่ | yes |
| GET    | /api/v1/money/      | list      | yes |
| GET    | /api/v1/money/{id}  | ดูตาม id  | yes |
| PATCH  | /api/v1/money/{id}  | แก้ไข     | yes |
| DELETE | /api/v1/money/{id}  | soft del  | yes |

## Domain Events

| Event | Trigger | Payload |
|---|---|---|
| MoneyCreated | after create | id, code, tenant_id |
| MoneyUpdated | after update | id, changes |
| MoneyDeleted | after delete | id, deleted_at |
...
## Testing
    ...
    pytest tests/integration/test_money_repository.py -v
    pytest --cov=app.modules.money --cov-fail-under=85

## Usage Example

    curl -X POST http://localhost:8000/api/v1/money/ \
