"""tests/unit/test_inventory_cache.py — Tombstone protocol."""
...
from app.modules.inventory.infrastructure.caches import (
    RedisInventoryCache,
    _entry_key,
    _tombstone_key,
)
...
@pytest.fixture
def cache(redis) -> RedisInventoryCache:
    return RedisInventoryCache(redis_client=redis)
...
class TestTombstoneProtocol:
    async def test_invalidate_writes_tombstone_before_delete(
        ...
        await cache.invalidate("inventory:x")
    ...
    async def test_set_suppressed_when_tombstone_present(
        ...
        await cache.set("inventory:x", entity)
