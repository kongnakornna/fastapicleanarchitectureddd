"""Inventory use cases — กรณีการใช้งาน inventory"""
...
class CreateInventoryUseCase:
    """TH: สร้าง inventory ใหม่ (idempotent) | EN: create (idempotent)"""
    ...
    async def execute(
        ...
        try:
            ...
            await self.cache.invalidate(f"inventory:{saved.id}")

            await self.idempotency.complete(
                key=idempotency_key, scope="inventory.create",
                tenant_id=str(self.ctx["tenant_id"]),
...
class GetInventoryUseCase:
    """TH: ดึง inventory ตาม id | EN: get by id"""
    ...
    async def execute(self, *, entity_id) -> Inventory:
        log.info("usecase.get.start", entity_id=str(entity_id))
        try:
            cached = await self.cache.get(f"inventory:{entity_id}")
            if cached is not None:
                return cached

            entity = await self.repo.get_by_id(entity_id)
            if entity is None or entity.is_deleted():
                raise InventoryNotFoundError(entity_id)

            await self.cache.set(f"inventory:{entity_id}", entity, ttl=300)
            return entity
        except StandardException:
            raise
        except DomainError as e:
...
class UpdateInventoryUseCase:
    ...
    async def execute(self, *, entity_id, **changes) -> Inventory:
        ...
        try:
            ...
            saved = await self.repo.save(entity)
            await self.cache.invalidate(f"inventory:{entity_id}")
...
class DeleteInventoryUseCase:
    """TH: ลบ inventory (soft) | EN: soft delete"""
