"""123Abc use cases — กรณีการใช้งาน 123abc"""
...
class Create123AbcUseCase:
    """TH: สร้าง 123abc ใหม่ (idempotent) | EN: create (idempotent)"""
    ...
    async def execute(
        ...
        try:
            ...
            await self.cache.invalidate(f"123abc:{saved.id}")

            await self.idempotency.complete(
                key=idempotency_key, scope="123abc.create",
                tenant_id=str(self.ctx["tenant_id"]),
...
class Get123AbcUseCase:
    """TH: ดึง 123abc ตาม id | EN: get by id"""
    ...
    async def execute(self, *, entity_id) -> 123Abc:
        log.info("usecase.get.start", entity_id=str(entity_id))
        try:
            cached = await self.cache.get(f"123abc:{entity_id}")
            if cached is not None:
                return cached

            entity = await self.repo.get_by_id(entity_id)
            if entity is None or entity.is_deleted():
                raise 123AbcNotFoundError(entity_id)

            await self.cache.set(f"123abc:{entity_id}", entity, ttl=300)
            return entity
        except StandardException:
            raise
        except DomainError as e:
...
class Update123AbcUseCase:
    """TH: แก้ไข 123abc | EN: update"""
    ...
    async def execute(self, *, entity_id, **changes) -> 123Abc:
        ...
        try:
            ...
            saved = await self.repo.save(entity)
            await self.cache.invalidate(f"123abc:{entity_id}")
...
class Delete123AbcUseCase:
    """TH: ลบ 123abc (soft) | EN: soft delete"""
