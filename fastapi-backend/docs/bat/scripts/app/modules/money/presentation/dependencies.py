"""Money dependencies — FastAPI DI container"""
...
from ..infrastructure.caches import RedisMoneyCache
from ..infrastructure.repositories import SQLAlchemyMoneyRepository
...
def get_money_repo(
    ...
    return SQLAlchemyMoneyRepository(session=session)


def get_money_cache() -> RedisMoneyCache:
    return RedisMoneyCache(redis_client=get_redis())


async def get_create_uc(
    repo: Annotated[SQLAlchemyMoneyRepository, Depends(get_money_repo)],
    cache: Annotated[RedisMoneyCache, Depends(get_money_cache)],
...
async def get_get_uc(
    ...
    return GetMoneyUseCase(repo=repo, cache=cache, ctx=ctx)
...
async def get_list_uc(
    ...
    return ListMoneyUseCase(repo=repo, ctx=ctx)
