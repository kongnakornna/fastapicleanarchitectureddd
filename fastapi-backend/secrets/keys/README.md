...
This folder contains the Ed25519 key pairs (private and public) required for the application's authentication system. These keys ensure the security, integrity, and confidentiality of the JSON Web Tokens (JWT).
...
-   **Signing Keys (`signing-private.pem` & `signing-public.pem`)**
    ...
    -   **Private Key**: Uses the `JWT_SIGNING_KEY_PASSWORD` to sign the token with ED25519 (EdDSA), certifying that it was issued by this application.
...
-   **Encryption Keys (`encryption-private.pem` & `encryption-public.pem`)**
    ...
    -   **Public Key**: Encrypts the token payload using ECDH-ES+A256KW so that sensitive data is hidden from the client.
...
2.  **Signing**: The payload is signed using the `signing-private.pem` key with ED25519 (EdDSA). This ensures the token cannot be tampered with without invalidating the signature.
...
The following commands use `openssl` to generate the required Ed25519 keys. They pull the passwords directly from your `.env` file variables (`JWT_SIGNING_KEY_PASSWORD` and `JWT_ENCRYPTION_KEY_PASSWORD`).
...
-   **Generate Private Signing Key**:
    ```bash
    openssl genpkey -algorithm ed25519 -out secrets/keys/signing-private.pem -aes256 -pass pass:$JWT_SIGNING_KEY_PASSWORD
    ```
...
-   **Generate Private Encryption Key**:
    ```bash
    openssl genpkey -algorithm ed25519 -out secrets/keys/encryption-private.pem -aes256 -pass pass:$JWT_ENCRYPTION_KEY_PASSWORD
    ```
