...
"""
TH: User aggregate root
EN: User aggregate root
"""
...
class User(BaseEntity):
    ...
    def __post_init__(self) -> None:
        ...
        # ── raise aggregated errors (workaround: single DomainError) ──
        if errors:
            # TH: ใช้ DomainError (single) แทน DomainErrors เพื่อเลี่ยง class พัง
            # EN: use single DomainError instead of broken DomainErrors
        ...
        for char in phone_str:
            if char.isdigit():
                if digit_index < len(censored_part):
                    ...
                    result.append(
                        visible_digits[digit_index - len(censored_part)]
                    )
                digit_index += 1
            else:
                result.append(char)
