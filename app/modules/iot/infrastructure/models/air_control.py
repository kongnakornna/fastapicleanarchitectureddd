...
class AirControl(BaseModelNoPK):
    __tablename__ = "sd_air_control"

    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), name="tenant_id", nullable=False, index=True)
    air_control_id: Mapped[int] = mapped_column(Integer, name="air_control_id", primary_key=True, autoincrement=True)
