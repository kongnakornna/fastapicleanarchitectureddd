"""AirSettingWarningDeviceMap — UUID PK"""
...
class AirSettingWarningDeviceMap(BaseModel):
    __tablename__ = "sd_air_setting_warning_device_map"

    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), name="tenant_id", nullable=False, index=True)
    air_setting_warning_id: Mapped[int | None] = mapped_column(Integer, name="air_setting_warning_id", nullable=True)
