...
class AirPeriod(BaseModelNoPK):
    __tablename__ = "sd_air_period"

    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), name="tenant_id", nullable=False, index=True)
    air_period_id: Mapped[int] = mapped_column(Integer, name="air_period_id", primary_key=True, autoincrement=True)
