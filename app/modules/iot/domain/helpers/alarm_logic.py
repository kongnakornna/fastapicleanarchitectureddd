...
Mirrors internal/modules/iot/iothelper/alarm.go.
"""
...
_THAI_MESSAGES: Final[Mapping[str, str]] = {
...
_ENGLISH_MESSAGES: Final[Mapping[str, str]] = {
    "warning": "Warning",
    "critical": "Critical",
...
def _normalize_sensor_value(value: Any) -> Any:
    """Mirror Go's normalizeSensorValue.

    - ``"ON"`` / ``"OFF"`` (case-insensitive) become ``1`` / ``0``.
...
def _is_one(value: Any) -> bool:
    """Mirror Go's isOneValue — numeric comparison to 1."""
    return _to_float(value) == 1


def _is_on_off_or_binary(value: Any) -> bool:
    """Mirror Go's isOnOffOrBinary — accepts 0, 1, ON, OFF."""
    if value is None:
        return False
    if isinstance(value, str):
        return value.upper() in ("ON", "OFF")
    return _to_float(value) in (0.0, 1.0)
...
def evaluate_alarm(
    ...
    data_alarm_raw = 0
    event_control = event
    message_mqtt_control = mqtt_control_on if event == 1 else mqtt_control_off
    ...
    if hardware_id == 3 and _is_on_off_or_binary(sensor_value):
        alarm_status_set = 999
        title = messages["normal"]
        subject = messages["normal"]
    ...
    elif hardware_id == 4 and not _is_one(sensor_value):
        alarm_status_set = 2
    ...
    elif hardware_id == 4 and _is_one(sensor_value):
        alarm_status_set = 999
        title = messages["normal"]
        subject = messages["normal"]
        content = f"{messages['normal']} {sensor_value} {unit}"
        ...
        if is_critical:
            ...
            title = messages["warning"]
        subject = (
            f"{mqtt_name} {title} : {device_name} : {sensor_value} {unit}"
        )
        content = (
